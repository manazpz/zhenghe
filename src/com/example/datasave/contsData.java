package com.example.datasave;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

import com.example.fragment.Socket.structScoket;
import com.example.hs.R;
import com.example.jsData.cjData;
import com.example.jsData.Struct;
import com.example.jsData.codeData;
import com.example.jsData.upbanben;
import com.smorra.asyncsocket.TcpClient;

public class contsData {
	public static final int[] img = new int[] {R.drawable.daojishi1, R.drawable.daojishi2,
			R.drawable.daojishi3,R.drawable.daojishi4,R.drawable.daojishi5,};
	
	public static HashMap<String, String> jhost = new HashMap<String, String>();
	public static HashMap<String, String> hhost = new HashMap<String, String>();
	public static List<String> data = new ArrayList<String>();
	public static ArrayList<upbanben> uplist = new ArrayList<upbanben>();
	public static ArrayList<codeData> codelist = new ArrayList<codeData>();
	public static ArrayList<codeData> codelist1 = new ArrayList<codeData>();
	
	
	public static String sername;
	public static String[] newpsw;
	public static String[] bankxx;
	public static String[] warger;
	
	public static structScoket janScoket = null;
	public static TcpClient tcp = null;

	
}
